...
def main(args):
    ...
    if not args["fqdn"].endswith(".fredhutch.org") and not args["fqdn"].endswith(
        ".fhcrc.org"
    ):
  ...
  {name}:
    ...
    deploy:
      ...
      labels:
        ...
        - "org.fredhutch.github.repo={github_repo}"
        - "traefik.enable=true"
...
in swarm-proxy:/etc/nginx/auth_repos.""",
    ...
    p.add_argument(
        ...
        default=3838,
        type=int,
        help="port exposed by default service",
    )
    p.add_argument(
        "-g",
        "--github-repo",
        default="NAME",
        action="store",
        help="github repo for this app (repo name only, omit https://github.com/FredHutch/)",
    )
    p.add_argument(
        "-a",
        "--auth-file-path",
        action="store",
        help="relative path to auth file (if using basic auth). Example: system/auth",
    )
    p.add_argument(
        "-o",
        "--overwrite",
    ...
    p.add_argument(
        "-r", "--owner-email", action="store", help="email of owner", required=True
    )
    p.add_argument(
        "-e",
        "--external",
    ...
    p.add_argument(
        ...
        "--no-websockets",
        action="store_true",
        help="does not use websockets, only needed if EXTERNAL==True",
    )
    args = p.parse_args()
